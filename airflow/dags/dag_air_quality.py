...
from mix_energy.air_quality_ingest import collect_city_csv_contents
from mix_energy.bucket_to_bigquery_airflow import run_transfer as _run_transfer

FILE_PREFIX = "air_quality"
...
def dag_air_quality():
    ...
    def ingest_csv_to_bucket(bucket_name: str) -> None:
        ...
        if not csv_contents:
            raise RuntimeError(
                "Recuperation air quality echouee ou aucune donnee disponible."
            )
        ...
        if uploaded_files == 0:
            raise RuntimeError(
                "Aucun fichier air quality n'a ete charge dans le bucket."
            )
    ...
    dbt_air_quality = BashOperator(
        task_id="dbt_air_quality",
