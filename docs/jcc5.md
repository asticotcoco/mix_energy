...
La bonne explication a donner au jury est la suivante : l'API est bien securisee, mais pas aujourd'hui par une API key applicative. La securisation actuellement visible dans le code repose sur quatre niveaux. Premier niveau, l'API est read-only pour la partie datasets, avec seulement des GET sur la consultation et aucun endpoint d'ecriture sur BigQuery. Deuxieme niveau, le CORS est restrictif et borne les origines autorisees. Troisieme niveau, les entrees sont validees strictement avec Pydantic et les filtres sont converts en requetes parametrees, ce qui limite les erreurs et les injections SQL. Quatrieme niveau, les volumes de donnees sont bornes par default_limit et max_limit pour eviter les appels trop lourds. Si le jury me demande une securisation par cle API, je peux repondre honnetement que ce n'est pas encore implemente dans cette version et que ce serait un axe d'industrialisation naturel.
